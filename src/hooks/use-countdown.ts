...
export function useCountdown(targetDate: string) {
  ...
  const dinamicCountdownText = () => {
    ...
    if (days > 0) {
      return `${formatTimeUnit(days, "dia", "dias")} ${formatTimeUnit(
        hours,
        "hora",
        "horas"
      )} restantes`
    ...
    if (hours > 0) {
      return `${formatTimeUnit(hours, "hora", "horas")} ${formatTimeUnit(
        minutes,
        "minuto",
        "minutos"
      )} restantes`
    ...
    return `${formatTimeUnit(minutes, "minuto", "minutos")} ${formatTimeUnit(
      seconds,
      "segundo",
      "segundos"
    )} restantes`
